...
import com.lfxwkj.sur.sys.core.constant.DefaultAvatar;
import com.lfxwkj.sur.sys.core.exception.enums.BizExceptionEnum;
import com.lfxwkj.sur.sys.modular.system.entity.FileInfo;
import com.lfxwkj.sur.sys.modular.system.entity.User;
import com.lfxwkj.sur.sys.modular.system.mapper.FileInfoMapper;
import com.lfxwkj.sur.sys.modular.system.model.UploadResult;
...
public class FileInfoService extends ServiceImpl<FileInfoMapper, FileInfo> {
     ...
     * @Date 2019-05-04 17:04
     */
    public String previewAvatar(String avatarId) {
        if (ToolUtil.isEmpty(avatarId)){
            return DefaultAvatar.BASE_64_AVATAR;
        } else {
            FileInfo fileInfo = this.getById(avatarId);
            if (fileInfo == null) {
                return DefaultAvatar.BASE_64_AVATAR;
            } else {
                try {
                    String filePath = fileInfo.getFilePath();
                    return Base64.encode(new File(filePath));
                } catch (Exception e) {
                    log.error("头像未找到！", e);
                    return DefaultAvatar.BASE_64_AVATAR;
                }
