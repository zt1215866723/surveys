...
import com.lfxwkj.sur.sys.modular.rest.mapper.RestDeptMapper;
...
public class RestDeptService extends ServiceImpl<RestDeptMapper, RestDept> {
    ...
    public List<RestDept> collegeList(String pid, Long deptId) {
        LoginUser user = LoginContextHolder.getContext().getUser();
        List<RestDept> list = this.baseMapper.collegeList(pid, deptId);
        if(list.size() == 0){
            RestDept restDept = this.getById(user.getDeptId());
            String[] pidArray = restDept.getPids().split(",");
            List<String> pidList = new ArrayList<>();
            for(String p : pidArray){
                if(!p.equals("")){
                    p = p.substring(1);
            ...
            QueryWrapper<RestDept> restDeptQueryWrapper = new QueryWrapper<>();
            restDeptQueryWrapper.in("dept_id",pidList);
            restDeptQueryWrapper.eq("dept_type",1);
            list = this.list(restDeptQueryWrapper);
        }
        return list;
    }
