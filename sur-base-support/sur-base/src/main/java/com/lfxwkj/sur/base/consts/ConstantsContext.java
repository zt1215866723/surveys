package com.lfxwkj.sur.base.consts;

import cn.stylefeng.roses.core.util.ToolUtil;
import com.lfxwkj.sur.base.enums.CommonStatus;
import com.lfxwkj.sur.base.sms.AliyunSmsProperties;
...
public class ConstantsContext {
     ...
     * @return
     */
    public static Map<String, Object> getDevUpdate(){
        Map<String, Object> map = new HashMap<String, Object>();
        Object ip = CONSTNTS_HOLDER.get("DEV_UPDATE_IP");
        Object url = CONSTNTS_HOLDER.get("DEV_UPDATE_URL");
        Object port = CONSTNTS_HOLDER.get("PORT");
        map.put("ip", ip);
        map.put("url", url);
        map.put("port", port);
        return map;
    ...
    /**
     *  获取课前的设备下发的提前时间， 课前小程序提醒时间，  课后删除时间
     * @return
     */
    public static Map<String, Object> getTipConfig(){
        Map<String, Object> map = new HashMap<String, Object>();
        Object DEV_CLASS_BEFORE = CONSTNTS_HOLDER.get("DEV_CLASS_BEFORE");
        Object DEV_CLASS_AFTER = CONSTNTS_HOLDER.get("DEV_CLASS_AFTER");
        Object XCX_CLASS_BEFORE = CONSTNTS_HOLDER.get("XCX_CLASS_BEFORE");
        map.put("DEV_CLASS_BEFORE", DEV_CLASS_BEFORE);
        map.put("DEV_CLASS_AFTER", DEV_CLASS_AFTER);
        map.put("XCX_CLASS_BEFORE", XCX_CLASS_BEFORE);
        return map;
    }
     ...
     * 二维码有效时间
     */
    public static Long getQsLong() {
        Long defaultSecs = 300000L;
        String systemReleaseVersion = (String) CONSTNTS_HOLDER.get("QS_LONG");
        if (ToolUtil.isEmpty(systemReleaseVersion)) {
            log.error("二维码有效时为空，采用默认值：5分钟" + TIPS_END);
            CONSTNTS_HOLDER.put("QS_LONG", String.valueOf(defaultSecs));
            return defaultSecs;
        } else {
            try {
                return Long.valueOf(systemReleaseVersion);
            } catch (NumberFormatException e) {
                log.error("二维码有效时间不合法，采用默认值：5分钟" + TIPS_END);
                CONSTNTS_HOLDER.put("QS_LONG", String.valueOf(defaultSecs));
                return defaultSecs;
            }
        }
